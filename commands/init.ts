...
class Init_TS {
    ...
    initialize(folderName: string) {
        console.log(chalk.blue(`Initializing TypeScript project in: ${folderName}`));
        try {
            execSync(`mkdir ${folderName}`, { stdio: "inherit" });
            execSync(`cd ${folderName} && npm init -y`, { stdio: "inherit" });
            execSync(`cd ${folderName} && npm install -D typescript ts-node nodemon @types/node`, { stdio: "inherit" });
            execSync(`cd ${folderName} && npx tsc --init`, { stdio: "inherit" });
            console.log(chalk.green(`TypeScript project ${folderName} initialized successfully!`));
        } catch (error) {
